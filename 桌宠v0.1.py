...
class DesktopPet(QWidget):
    ...
    def init_ui(self):
        ...
        self.history_button.setGeometry(self.config["window_size"][0] - 50, 50, 40, 30)
        ...
        self.image_label.setGeometry(0, 160, self.config["window_size"][0], self.config["window_size"][1] - 200)
        ...
        self.restart_button.setGeometry(self.config["window_size"][0] - 50, self.config["window_size"][1] - 40, 40, 30)
    ...
    def update_image(self, mood):
        try:
            ...
            pixmap = pixmap.scaled(self.config["window_size"][0], self.config["window_size"][1] - 200,
        ...
        try:
            ...
            if match:
                ...
                if mood in self.moods:
                    ...
                    # 仅显示桉树回复（无前缀）
                    self.dialog_text.setPlainText(chinese.strip())
                    # 记录完整对话历史
                    ...
                    logging.info(f"Ollama response: {mood} | {chinese.strip()}")
                else:
                    logging.warning(f"Invalid mood in response: {mood}")
                    self.update_image("无奈")
                    self.dialog_text.setPlainText(f"错误：无效心情 {mood}")
            else:
                logging.error(f"Failed to parse Ollama response: {output}")
                self.update_image("无奈")
                self.dialog_text.setPlainText("错误：无法解析回复")
        except Exception as e:
            logging.error(f"Error in query_ollama: {e}")
            self.update_image("无奈")
            self.dialog_text.setPlainText(f"错误：{str(e)}")
