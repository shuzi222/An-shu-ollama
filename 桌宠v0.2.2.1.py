...
from PyQt5.QtCore import Qt, QEvent, QTimer, QPropertyAnimation, pyqtSignal
...
class DesktopPet(QWidget):
    ...
    def __init__(self):
        ...
        self.default_config = {
            ...
            "sensevoice_api_key": "",
            "image_path": os.path.join(os.path.dirname(__file__), "An-shu"),  # 图片路径改为当前目录下的 An-shu
            "window_alpha": 1.0,
            "window_size": (400, 732),
            "record_key": "f7",  # 默认录音按键为 F7
            "persona": (
        ...
        self.interaction_timer.start(60000)  # 60秒无交互触发淡出
        ...
        try:
            ...
            if key_event.event_type == keyboard.KEY_DOWN:
                ...
                if key:
                    ...
                    self.config["record_key"] = key  # 立即更新配置
    ...
    def process_audio(self, frames):
        wav_path = os.path.join(os.path.dirname(__file__), "temp.wav")  # 临时音频文件保存到当前目录
        try:
            if not frames:
